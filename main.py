...
/skills - добавление навыка проекту
...
def description_handler(message):
    projects = [x[2] for x in manager.get_projects(message.from_user.id)]
    if not projects:
        bot.send_message(message.chat.id, "У тебя нет проектов!")
        return
    if message.text in projects:
        bot.send_message(message.chat.id, "Введи описание:")
        bot.register_next_step_handler(message, lambda msg: manager.update_projects('description', (msg.text, message.text, msg.from_user.id)) or bot.send_message(msg.chat.id, "✅ Описание добавлено!"))
    else:
        bot.send_message(message.chat.id, "Выбери проект:", reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, description_handler)
